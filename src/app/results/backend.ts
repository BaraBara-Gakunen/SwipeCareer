...
import { Company, CompanyResult, RecommendationCompany, FavoriteCompaniesAnalysis, MatchedCompany, SelfAnalysis} from "@/types/types";
...
export const calcAndGetRecommendationCompany = async (): Promise<RecommendationCompany[]> => {
  ...
  const recommendations: RecommendationCompany[] = await Promise.all(
    top5Companies.map(async (company) => {
      try {
        const response = await fetch("/results/api", {
          method: "POST",
          headers: {
